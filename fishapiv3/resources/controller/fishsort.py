...
def createFishTransfer(origin_activation, destination_activation, args, transfer):
    return FishTransfer(**{
        ...
        "fish_seed_id": args["fish_seed_id"],
...
def calculatedFishRemains(args, fish_transfer):
    origin_activation = PondActivation.objects(pond_id=args['origin_pond_id'], isFinish=False).order_by('-activated_at').first()
    fish_alive = []
    fish_alive_obj = FishLog.objects.aggregate([{'$match': {
                                '$expr': {'$and': [
...
class FishSortsApi(Resource):
    ...
    def post(self):
        # try:
            ...
            parser.add_argument("fish_seed_id",type=str, required=True, location='form')
            ...
            parser.add_argument('image', type=str, required=False, default="-", location='form')
            parser.add_argument('width', type=str, required=False, default="-", location='form')
            # print("", location='form')
            ...
            if (args['fish_sort_type']== 'basah'):
                ...
                # ke inventrory
                    ...
                    # hitung jumlah ikan skrg
                fish_summary = calculatedFish(transfer_list)
                print("fish summary:", fish_summary)
                # hitung sisa ikan yg akan dimasukkan ke inventory
                fish_remains_summary = calculatedFishRemains(args,fish_summary)
                print("fish remains summary:", fish_remains_summary)
                tempamount = fish_remains_summary[0]['amount']
                tempprice = args['price']
                temp = tempamount * tempprice
                print("remaining fish that inserted into inventory = ", tempamount)
                print("each fish price = ", tempprice)
                print("total amount = ", temp)
                # input ke inventory benih
                for item_fish in transfer['fish']:
                    current_user = get_jwt_identity()
                    farm = str(current_user['farm_id'])
                    body = {
                        "farm_id": farm,
                        "fish_seed_category": args["fish_category"],
                        "fish_type": item_fish['type'],
                        "brand_name": "transfer_in",
                        "amount": fish_remains_summary[0]['amount'],
                        "weight": item_fish['weight'],
                        "width": "0",
                        "price": args['price'],
                        "total_price": fish_remains_summary[0]['amount'] * args['price'],
                        "image": "-",
                        "fish_origin_category" : args["origin_pond_id"]
                    }
                    print("body seed inventory in fishsort.py: ", body)
                    inventory = SeedInventory(**body).save()
                
                # deactivation origin pond
                print("menonaktifkan pond....")
